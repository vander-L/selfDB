...
import org.yujiabin.selfDB.exception.DeadLockException;
...
public class LockTable {
    ...
    private Lock lock;
    private Map<Long, Integer> xidStamp;//XID对应的时间戳
    private int stamp;
    ...
    /**
     * 往图中添加一条边，并判断是否会造成死锁，会造成死锁则抛出异常
     * @param xid 事务
     * @param uid 资源
     * @return 需要等待则返回null，否则返回锁对象
     */
    ...
    /**
     * 将uid1从列表listMap.get(uid0)中移除；若列表的数量为0，则将列表也从Map中移除
     */
    ...
    /**
     * 将uid1添加到列表listMap.get(uid0)中；若Map中不含有该列表，则先添加列表到Map中
     */
