...
public class PageCacheTest {
    ...
    public void testPageCache() throws Exception {
        PageCache pc = PageCache.createDbFile("/tmp/pcacher_simple_test0", PageCache.PAGE_SIZE * 50);
        for(int i = 0 ; i < 100; i ++) {
            byte[] tmp = new byte[PageCache.PAGE_SIZE];
            tmp[0] = (byte)i;
            int pgno = pc.createNewPage(tmp);
        ...
        pc = PageCache.openDbFile("/tmp/pcacher_simple_test0", PageCache.PAGE_SIZE * 50);
        for(int i = 1; i <= 100; i ++) {
    ...
    public void testPageCacheMultiSimple() throws Exception {
        pc1 = PageCache.createDbFile("/tmp/pcacher_simple_test1", PageCache.PAGE_SIZE * 50);
    ...
    private void worker1(int id) {
        for(int i = 0; i < 80; i ++) {
            ...
            if(op == 0) {
                byte[] data = RandomUtil.randomByteArray(PageCache.PAGE_SIZE);
                int pgno = pc1.createNewPage(data);
                Page pg = null;
                try {
                    pg = pc1.getPage(pgno);
                } catch (Exception e) {
    ...
    public void testPageCacheMulti() throws InterruptedException {
        pc2 = PageCache.createDbFile("/tmp/pcacher_multi_test", PageCache.PAGE_SIZE * 10);
    ...
    private void worker2(int id) {
        for(int i = 0; i < 1000; i ++) {
            ...
            if(op == 0) {
                ...
                byte[] data = RandomUtil.randomByteArray(PageCache.PAGE_SIZE);
                lockNew.lock();
                int pgno = pc2.createNewPage(data);
                int mpgno = mpc.createNewPage(data);
                assert pgno == mpgno;
                lockNew.unlock();
                ...
                byte[] newData = RandomUtil.randomByteArray(PageCache.PAGE_SIZE);
