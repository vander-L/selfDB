...
public class TransactionManagerTest {
    ...
    public void testMultiThread() {
        tmger = TransactionManager.createXidFile("/tmp/transactionManagerTest");
        ...
        tmger.close();
        assert new File("/tmp/transactionManagerTest.tran").delete();
    }
        ...
        for(int i = 0; i < noWorks; i ++) {
            ...
            if(op == 0) {
                ...
                if(inTrans == false) {
                    ...
                    switch (status) {
                        case 1 -> tmger.commit(transXID);
                        case 2 -> tmger.abort(transXID);
                    }
                ...
                if(transCnt > 0) {
                    ...
                    boolean ok = switch (status) {
                        case 0 -> tmger.isActive(xid);
                        case 1 -> tmger.isCommitted(xid);
                        case 2 -> tmger.isAborted(xid);
                        default -> false;
                    };
                    assert ok;
                }
